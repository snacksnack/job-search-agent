...
- `reassessSuggested`: true when the role's JD changed after it was assessed. The cached `skillMatch` is kept and keeps feeding `matchPercent`; the role re-enters `skill_match.py --list-pending` for a deliberate re-assessment, which clears the flag.
...
Every role carries `contentHash` — `sha256(title|location|description)[:16]`, refreshed at each pipeline write. When the sweep re-sees a posting that's already stored (matched by id, url, or company+title), it classifies it: **unchanged** → skipped as before; **changed** (JD rewritten, location moved, salary added) → the stored role is updated in place and stamped `changedDate`. For roles the user hasn't decided on, a changed JD also marks the cached `skillMatch` stale and rescores; a real assessment (one stamped `assessedBy`/`model`, or pinned with `assessmentLocked`) is **kept** and the role is flagged `reassessSuggested: true` so it re-enters `skill_match.py --list-pending` — deleting it would drop the fit back to the neutral baseline and silently inflate `matchPercent` to an unassessed heuristic. Only an unstamped cached block is dropped outright. Roles already decided in `state.json` get the data update only, with no re-assessment churn, and are never resurfaced. Guards: an empty incoming description (list-only SmartRecruiters/Gem rows) or one under half the stored length never counts as a JD change, and whitespace differences are ignored. The run entry records `changedRoles`: `{"active": ["Company — Title", ...], "decided": [...], "reassessing": N}` — mention changed roles in the briefing's run stats, and call out `decided` entries (a JD that changed after you applied is worth knowing).

### Manual-check reminders (unfetchable boards)

Watchlist entries without a working fetcher+slug (e.g. `ats: workday` with a `boardUrl` and optional `cadence`: `weekly` (default) / `biweekly` / `monthly`) become cadence-based reminders instead of silently falling off the radar. When a company's cadence has elapsed, the pipeline prints a `CHECK <company> ...` line, records the batch as `manualChecks` on the run entry, and stamps `lastReminded` in `data/manual-check-state.json` (local, gitignored; not stamped on dry runs). Surface due reminders in the briefing — checking the board is a HUMAN act, or at most a WebSearch-snippet pass; NEVER a logged-in/Workday browse.

### Title-rescue queue (data/queue/title-rescue.json)

Fresh postings rejected ONLY by the title filter are saved each run as `{"date", "roles": [normalized role objects, JD capped at 2000 chars]}` (queue replaced per run, capped at 400). The daily run reviews the distinct titles via `scripts/title_rescue.py --list-pending` / `--apply` (see SKILL.md run order step 4). Rescued roles carry `titleRescued: true`, re-enter the remaining filters, rank normally, and render with an amber "Rescued" badge ("double-check role scope"). Mention rescued roles in the briefing's top-roles or run-stats section.
...
6. **Run stats** — compact line: searches run, found/reviewed/qualified totals, skip breakdown, expired listings, and — when nonzero — how many listings were marked closed and how many re-seen postings changed this run (call out by name any closed or changed role the user had applied to or was interviewing for).
7. **Source health** — only when the pipeline reported fetch failures: one line per failed watchlist board (company, ats/slug, error) so a dead slug gets noticed the same day, with the fix called out for any 3+ run streak (re-run `--resolve-ats` or update the slug in `search.json`). Empty-but-successful boards need no line item; a count is enough. Omit this section entirely when every board fetched. If the run printed `Manual-check reminders`, list each due board here (company + boardUrl + how long since last reminded) as an action item for the user — a human check or a WebSearch-snippet pass, never a logged-in/Workday browse.
