...
2. **LinkedIn discovery (Claude in Chrome).** For each enabled `linkedin`/discovery search in `search.json`, browse and capture raw postings, then write them to `data/inbox/linkedin-<date>.json` (schema in `references/output-schemas.md` → "Inbox finds"). Do not score or filter in the chat — that is the pipeline's job. See `references/search-execution.md`.
   ...
   > **Standing rule — stop on LinkedIn throttling.** At any sign LinkedIn is throttling or rate-limiting (HTTP 429, a "you've reached the commercial use limit"/"unusual activity" interstitial, a CAPTCHA/checkpoint/login wall, repeated empty results, or pages that suddenly stop loading), **stop the LinkedIn browsing immediately.** Do not retry, switch accounts, or attempt to work around the block. Save whatever was captured so far to the inbox, tell the user LinkedIn was throttling and that you stopped, and continue with the non-LinkedIn parts of the run (pipeline ingest of what's already captured, ATS sweep, etc.). The user does not need to remind you of this each time — treat it as always-on. Prefer light-touch capture (read posting links/IDs from the saved-jobs or results list rather than clicking into every card) to avoid tripping limits in the first place.
3. **Run the pipeline.** Invoke `python3 scripts/pipeline.py` via Bash. It performs the watchlist sweep (Greenhouse/Lever/Ashby APIs), ingests `data/inbox/*.json`, enriches ATS-hosted postings, deduplicates against `jobs.json` + `state.json` (by url, id, and company+title, so a posting already on the board from another source is merged rather than duplicated — the `Cross-source dedup:` line reports those), applies all filters (title, employer, salary, location, description) and scoring per `references/scoring-and-filters.md`, then writes new roles to `data/jobs.json` and appends a run entry to `data/search-log.json`. Read its stdout summary for the counts, including the `Source health:` block — any `FETCH FAILED` line is a broken fetch (renamed slug, ATS migration), not an empty board, and must be surfaced in the morning briefing's Source health section (see `references/output-schemas.md`). The `Closed-listing check:` line reports roles marked closed because they vanished from a successfully-fetched board; mention the count in the briefing's run stats, and call out by name any closed role the user had applied to. The `Change detection:` lines report re-seen postings whose content changed (JD edited, salary added, location moved) — updated roles pending re-assessment surface via the normal skill-match flow, and any `changed (decided)` line means a posting changed after the user acted on it, which belongs in the briefing. `Manual-check reminders:` lines list watchlist companies on unfetchable boards (Workday etc.) whose check cadence has elapsed — surface them in the briefing as action items for the user; a WebSearch-snippet pass is the most you may do yourself, never a logged-in or Workday browse.
4. **Title-rescue review (inline, cheap).** The pipeline saves fresh title-rejected postings to `data/queue/title-rescue.json`. Run `python3 scripts/title_rescue.py --list-pending` — it prints the DISTINCT rejected titles (with counts and sample companies; no JDs). Review them yourself, conservatively: approve ONLY titles a senior program / delivery / solutions / customer-engineering candidate would credibly hold — nonstandard phrasings of in-scope roles ("PgM III", "Program Mgr", "Technical Programme Manager", "Delivery Lead" variants). NEVER approve software-engineering, product, marketing, sales, support, success, design, or people-leadership (Director+) titles — those are filtered on purpose. Expect to approve few or none most days; an empty approval is a fine outcome. Then write `{"approved": ["<exact title>", ...]}` to a temp file and run `python3 scripts/title_rescue.py --apply <file>` — approved postings re-enter the cascade (employer/salary/location/description checks still apply), get flagged `titleRescued`, and appear on the board with a "Rescued" badge. Mention any rescued roles in the briefing. Failure at any point is non-fatal — skip and continue; the queue is rebuilt next run.
5. **Skill-match assessment (Sonnet subagent).** This is the only LLM cost in the run, and it happens once per role (cached thereafter). See "Skill-match assessment" under Agentic steps. Skip if `skill_match.py --list-pending` returns nothing.
6. **Self-expanding discovery** (if `agentic.selfExpandingDiscovery`): see "Agentic steps" below. New companies are appended to `search.json.watchlist` so the pipeline monitors them directly on the next run.
7. **Company briefs** (if `agentic.companyBriefs`): generate briefs for the top {topN} new roles (see "Agentic steps").
8. **Self-tuning** (if `agentic.selfTuningMatching`): analyze `state.json` and propose matching adjustments (see "Agentic steps"). Propose — do not silently apply.
9. **Morning briefing.** Output the briefing (see `references/output-schemas.md` → "Morning briefing"). Point the user to the local board: `python3 scripts/serve.py` → http://127.0.0.1:8000.
...
Shows, per role on the board, how the candidate's resume skills/experience line up with what the JD asks for. **Done by an LLM once per role, then cached** on the role as `skillMatch` in `jobs.json` — never recomputed on later runs (the pipeline dedups, so a role is "pending" exactly once). An assessment is never deleted by a pipeline run: when a role's JD changes under it (re-seen posting, re-enrich, browser enrichment), the assessment and the score it produced are kept and the role is flagged `reassessSuggested: true`, which puts it back in the pending list for a deliberate re-assessment.

1. `python3 scripts/skill_match.py --list-pending` prints the roles that need assessing — those with no `skillMatch` yet, plus those flagged `reassessSuggested` because their JD changed (id, company, title, location, trimmed description). If empty, skip this step.
...
Roles with thin descriptions can be upgraded first with `python3 scripts/pipeline.py --reenrich` (refetches full JDs for roles that have a Greenhouse/Lever/Ashby `atsUrl`, rescores, and flags their cached `skillMatch` stale so they re-assess on the fuller text). LinkedIn/source-only roles have no ATS url and can't be re-enriched this way — use browser enrichment (below) for those.
...
3. Assemble results as JSON: a list of `{"id", "fullDescription", "salaryMin"?, "salaryMax"?, "location"?, "postedDate"?}`. Write to a temp file and merge with `python3 scripts/web_enrich.py --apply <file>`. It rescores, flags the role `enrichedVia: "web"`, and marks any cached `skillMatch` stale (`reassessSuggested`) so the role re-assesses without losing the existing assessment.
